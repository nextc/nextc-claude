...
Read `references/product-explorer-modes.md` when the invocation flag is any of:
`--fast`, `--update`, `--branch`, `--deep-dive`, `--export`

The deep mode pipeline below is the default path (no flag, or `--quick`/`--no-collision`/`--auto` only).
