...
**doc-keeper is now approval-gated, sonnet-powered, and bound to real data.** The `doc-keeper` agent upgraded from haiku/medium to sonnet/high and no longer auto-spawns: the `project-docs` rule's "spawn at the end of any code-changing response" flow is replaced with suggest-then-approve — a doc sync is suggested only at proper completion points (feature completely built, milestone done, fix confirmed, architecture decision finalized) and spawns only after the user says yes; otherwise doc updates stay user-driven via `/update-docs` (whose invocation counts as approval). Both the agent and the skill gained a "Real Data Only (CRITICAL)" mandate: every fact written to docs must be verified in-session against actual git output, file contents, or command output — no invented numbers, dates, versions, paths, API shapes, or behavior; unverifiable values are marked `*(unverified — needs check)*` or omitted. All auto-spawn echoes synced: `feature-dev`/`bug-fix`/`cleanup`/`team-feature-dev` Phase 8 doc steps now suggest instead of spawn, `git-workflow` pre-commit no longer spawns doc-keeper unasked, and the roster, project CLAUDE.md, and `/validate`'s `MODEL_TIER_TABLE` (schema.js) all record the new sonnet tier.
