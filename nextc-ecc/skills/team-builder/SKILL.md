...
1. **`claude agents` command** (primary) — run `claude agents` to get all agents known to the CLI, including user agents, plugin agents, and built-in agents. This automatically covers marketplace installs without any path configuration.
...
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `some-plugin:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
