...
This skill does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against available coverage
...
- Prefer skills and agents already available in the installed plugin set before recommending new installs.
...
- Separate three things clearly:
  ...
  - available but not wrapped in a clean operator workflow
...
1. Repo surface
   ...
   - `.mcp.json`, `.lsp.json`, `.claude/settings*.json`
...
4. Available skill/agent surface
   - Existing skills, commands, hooks, and agents that already cover the need
   - Check installed nextc-claude marketplace plugins: nextc-core, nextc-product, nextc-project-kickoff, nextc-flutter, nextc-aso
...
- "Stripe is available via connected app, but there is no billing-operator skill installed"
- "Google Drive is connected, but there is no Google Workspace operator workflow"
...
2. whether an installed skill/agent already has parity
3. whether there are only primitives available
4. whether the workflow is missing entirely

### Phase 3: Turn Gaps Into Decisions

For every real gap, recommend the correct shape:
...
Default to user-facing skills that orchestrate existing tools when the need is operational rather than infrastructural. Check whether a nextc-claude marketplace plugin already covers the gap before recommending a net-new build.
...
2. **Parity**
   - where installed skills/agents already match or exceed the benchmark
3. **Primitive-only gaps**
   - tools exist, but there is no clean operator skill wrapping them
...
- If there is already a strong primitive available, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what should be added next.
